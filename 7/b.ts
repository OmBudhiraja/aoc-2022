...
const availabeDirsToDelete: number[] = [];
...
const requiredSpace = tree.size - 40000000;
...
function visitEachDir(node: FsTree) {
  ...
  for (const item of node.items) {
    if (item.items) {
      item.size >= requiredSpace && availabeDirsToDelete.push(item.size);
      visitEachDir(item);
    }
  }
}
...
console.log(Math.min(...availabeDirsToDelete));
